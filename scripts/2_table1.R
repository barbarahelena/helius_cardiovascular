...
table1 <- helius %>%
    dplyr::select(Age_baseline, Sex, EthnicityTot, MigrGen, FUtime, BMI_baseline, Smoking_baseline, AlcCons_baseline, 
           DM_baseline, SBP_baseline, DBP_baseline, HT_BPMed_baseline, Dyslipidemia_baseline, 
    ...
    print(nonnormal=c("SCORECVDmortNL_baseline", "Trig"), noSpaces = TRUE, 
...
table1 <- helius %>% 
    dplyr::select(Age, Sex, EthnicityTot, MigrGen, FUtime, BMI, Smoking, AlcCons, 
           ...
           timepoint) %>% 
    CreateTableOne(data=., strata = 'timepoint', test = FALSE, addOverall = TRUE) %>% 
